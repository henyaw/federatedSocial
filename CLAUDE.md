...
- **An image's `only-once` first-time-setup markers can gate app state behind an unrelated feature flag.** The pixelfed-glitch image runs `passport:keys` under `OAUTH_ENABLED` but `passport:client --personal` only under `PF_LOGIN_WITH_MASTODON_ENABLED`, so OAuth looked configured while `/settings/applications` 500'd with "Personal access client not found." Fixed with an idempotent `bootstrap.sh provision-pixelfed` (auto-run by `up pixelfed`), not by flipping the unrelated flag. Also note the general hazard: those markers live in the app's storage volume while the state they claim to have created lives in Postgres — reprovisioning the DB with the volume intact makes the setup step silently never re-run. Repo-side provisioning should re-check the DB, not trust a marker.
